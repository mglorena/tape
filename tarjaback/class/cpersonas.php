...
class Persona {
    ...
    var
            $Nombre = '',
            $Apellido = '', $Domicilio = '', $ArticuloId = '', $Observaciones = '', $Fecha = 'NULL',
            $CargoDesc = '', $Categoria = '', $CantHoras = '', $Desde = 'NULL', $Hasta = 'NULL',
            $Compensatorio = 'NULL', $CP = 'NULL', $LicAnt = 'NULL', $LicA = 'NULL', $Total = 'NULL',
            $Compensatorios = 'NULL',
            $Legajo = 'NULL',
            $Active = 'NULL';
...
//            $e = new Errors();
//            $e->SendErrorMessage(new Exception("depurando errores2222"), "personas.php - GrabarPersona", $this->$key);
        }
    }

    function Compesatorios() {
//        $e = new Errors();
//        $e->SendErrorMessage(new Exception("depurando errores"), "personas.php - GrabarPersona", $this->Compensatorios);
        if ($this->DeleteCompensatorios()) {
//            $e = new Errors();
//            $e->SendErrorMessage(new Exception("depurando errores2"), "personas.php - GrabarPersona", $this->Compensatorios);
            foreach ($this->Compensatorios as $value) {
//                $e = new Errors();
//                $e->SendErrorMessage(new Exception("depurando errores3"), "personas.php - GrabarPersona", $value);
                if (!$this->InsertCompensatorio($value[0], $value[1], $value[2], $value[3])) {
                    return false;
                }
            }
    ...
    function InsertCompensatorio($fecha, $entrada, $salida, $isHabil) {
        $query = "";
        try {
    ...
    function DeleteCompensatorios() {
        ...
        $query = "";
        try {
            $query .="call personas_delCompe(" . $this->PersonaId . ");";
            $db = new sqlprovider();
            $db->getInstance();
            $db->setQuery($query);
            $result = $db->execute();
    ...
    function Guardar() {
        ...
        try {
            ...
            $query = "call personas_update(" . $this->PersonaId . ",'" . $this->Apellido . "','" . $this->Nombre . "','" . $this->Legajo . "','" . $this->CargoDesc . "'," . $this->Categoria . ",'" . $this->Domicilio . "'," . $this->VacacionesDias . ");";
    ...
    function GuardarDesc() {
        ...
        try {
            ...
            $query = "call personas_updateObs(" . $this->PersonaId . ",'" . $this->Observaciones . "');";
            $db = new sqlprovider();
            $db->getInstance();
            $db->setQuery($query);
            if ($db->execute() == "1") {
    ...
    function Search() {
        ...
        try {
            ...
            $db->CloseMysql();
            /* $e = new Errors();
              $e->SendErrorMessage(new Exception("update act"), "cpersonas.php - Save", $query); */
            return $personas;
        } catch (Exception $ex) {
            $e = new Errors();
            $e->SendErrorMessage($ex, "cpersonas.php - Search", $query);
